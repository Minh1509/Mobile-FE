...
import { collection, doc, getDoc, setDoc, updateDoc, arrayUnion, getDocs, query, where, Timestamp } from 'firebase/firestore';
...
export const saveBankSmsToFirestore = async (userId: string, parsedSms: ParsedSms): Promise<void> => {
    ...
    if (docSnap.exists()) {
        ...
        const isDuplicate = existingTransactions.some((tx: any) => tx.id === transactionId);
    ...
    const transaction: Omit<ITransaction, 'id' | 'type' | 'createdAt' | 'updatedAt'> = {
        userId,
        amount: parsedSms.amount ? Math.abs(parsedSms.amount) : 0,
        date: typeof parsedSms.date === 'string' ? parsedSms.date : formatDate(parsedSms.date || new Date()),
        time: parsedSms.time || new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' }),
        category: "Khác",
