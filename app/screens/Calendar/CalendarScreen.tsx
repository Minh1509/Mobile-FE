...
export default function CalendarScreen() {
  ...
  const { categoryData, incomeTotal, expenseTotal } = useMemo(() => {
    if (loading || !transactions.length)
      return { categoryData: {}, incomeTotal: 0, expenseTotal: 0 };
      ...
      <View className="flex-1">
        <View className="flex-row justify-between mx-4 mt-2 mb-2">
          {[
            ...
            <View
              key={label}
              className="w-[48%] bg-white p-4 rounded-lg shadow"
            >
              <View className="flex-row items-center mb-2">
                <Ionicons
                  name={icon as "add-circle" | "remove-circle"}
                  size={24}
                  color={color === "green" ? "#34C759" : "#FF3B30"}
                />
                <Text className={`text-${color}-600 font-bold ml-2 text-base`}>
                  {label}
                </Text>
              </View>
              <Text
                className={`text-${color}-600 font-bold text-base`}
                numberOfLines={2}
              >
          ...
          {Object.entries(categoryData).map(([category, { amount, type }]) => {
            ...
            return (
              ...
              >
                  ...
                  <Text className="text-gray-500">
                    {
                      transactions.filter(
                        (t) =>
                          normalizeDate(t.date) === selectedDate &&
                          t.category === category
                      ).length
                    }{" "}
                    giao dịch
                ...
                <Text
                  className={`text-base font-bold ${isExpense ? "text-red-500" : "text-green-500"}`}
                >
                  {isExpense ? "- " : "+ "}
