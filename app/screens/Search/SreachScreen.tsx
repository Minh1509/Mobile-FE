...
const SearchScreen = () => {
  const navigation =
    useNavigation<
      StackNavigationProp<RootStackParamList, "TransactionDetail">
    >();
  ...
  const filteredTransactions = useMemo(() => {
    ...
    let queryLower = searchQuery.trim().toLowerCase();

    return transactions
      .filter(({ description = "", category = "", date = "", amount }) => {
        const queryLower = searchQuery?.toLowerCase?.() || "";
        const fieldsToSearch = [description, category, date, String(amount)];
      ...
      {isLoading ? (
        ...
        <ScrollView
          ...
          contentContainerStyle={{
            ...
            paddingVertical: 12,
            paddingBottom: 24,
            flexGrow: filteredTransactions.length ? undefined : 1,
          }}
        >
