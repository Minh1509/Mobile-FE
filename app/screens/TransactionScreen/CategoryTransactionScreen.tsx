...
const CategoryTransactionsScreen = () => {
  ...
  const categoryTransactions = useMemo(() => {
    ...
    return transactions.filter(({ date, category: tCategory }) => {
      ...
      const matchesCategory = tCategory === category;
      const matchesMonthYear = tMonth === month && tYear === year;
      const matchesSelectedDate = selectedDate
        ? date === selectedDate.split("-").reverse().join("/")
        : true;

      return matchesCategory && matchesMonthYear && matchesSelectedDate;
    });
  }, [transactions, loading, category, month, year, selectedDate]);
